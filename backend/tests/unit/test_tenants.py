...
async def test_create_tenant_returns_created_tenant(monkeypatch) -> None:
    t = _tenant()
    monkeypatch.setattr("app.api.tenants.tenant_repo.get_by_slug", AsyncMock(return_value=None))
    monkeypatch.setattr("app.api.tenants.tenant_repo.create", AsyncMock(return_value=t))

    result = await create_tenant(TenantCreate(name="Acme", slug="acme"), _manager(), _mock_session())
...
async def test_create_tenant_raises_409_on_duplicate_slug(monkeypatch) -> None:
    monkeypatch.setattr("app.api.tenants.tenant_repo.get_by_slug", AsyncMock(return_value=_tenant()))

    with pytest.raises(HTTPException) as exc_info:
        await create_tenant(TenantCreate(name="Acme", slug="acme"), _manager(), _mock_session())
...
async def test_list_tenants_returns_all_tenants(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.api.tenants.tenant_repo.list_all", AsyncMock(return_value=tenants))
...
async def test_list_tenants_returns_empty_list(monkeypatch) -> None:
    monkeypatch.setattr("app.api.tenants.tenant_repo.list_all", AsyncMock(return_value=[]))
...
async def test_suspend_tenant_returns_suspended_tenant(monkeypatch) -> None:
    t = _tenant()
    t.is_active = False
    monkeypatch.setattr("app.api.tenants.tenant_repo.suspend", AsyncMock(return_value=t))
...
async def test_suspend_tenant_raises_404_when_not_found(monkeypatch) -> None:
    monkeypatch.setattr("app.api.tenants.tenant_repo.suspend", AsyncMock(return_value=None))
